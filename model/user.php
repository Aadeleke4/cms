...
if (isset($_POST['submit'])) {
    ...
    if ($thisUser['password'] == $pass) {
        ...
        if (isset($uname) && $uname != $thisUser['name']) {
            $sq .= "name = '$uname',";
        }

        if (isset($_POST['email']) && $_POST['email'] != $thisUser['email']) {
            $uEmail = $_POST['email'];
            $sq .= "email = '$uEmail',";
        }
        ...
        if (isset($_FILES['uavtr'])) {
            ...
            if ($file['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($format, $allowedFormats)) {
                    $location = 'Users/' . uniqid() . '.' . $format;

                    if (move_uploaded_file($file['tmp_name'], $location)) {
                        $sq .= "avatar='$location',";
                    } else {
                        $m = "Failed to move uploaded file.";
                    }
                } else {
                    $m = "Invalid image format. Allowed formats: " . implode(', ', $allowedFormats);
                }
            } else {
                $m = "File upload error: " . $file['error'];
            }
        }
        ...
        if (isset($_POST['npass']) && $_POST['npass'] != '' && isset($_POST['cpass']) && $_POST['cpass'] != '') {
            if ($_POST['npass'] == $_POST['cpass']) {
                ...
                if ($newPass != $thisUser['password']) {
                    $sq .= "password= '" . md5($newPass) . "',";
                }
            }
        }
                                    ...
                                    if(mysqli_num_rows($res)>0) {
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            ...
                                            if ($thisUser['is_admin'] == 1) {
                                                ...
                                                echo '<td>
                                                        <a href="viewUser.php?id=' . $row['id'] . '" class="btn btn-success btn-sm">
                                                            <span class="glyphicon glyphicon-eye-open"></span>
                                                        </a>
                                                        <a href="deleteuser.php?id=' . $row['id'] . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure you want to delete this user?\')">
